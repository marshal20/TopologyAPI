...
TEST(TopologyAPI, Topology)
{
	...
	top.add_component(std::shared_ptr<NMOS>(new NMOS("M1", { 1.5, 1, 2 }, "VDD", "VIN", "VO")));
	...
	EXPECT_NO_THROW(top.get_component_with_id("M1"));
	EXPECT_ANY_THROW(top.get_component_with_id("M2"));
